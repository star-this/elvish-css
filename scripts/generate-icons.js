/**
 * Generate individual icon CSS files for tree-shakeable imports
 * 
...
const ICONS_DIR = path.join(__dirname, '../global/icons');
...
for (const [name, dataUri] of Object.entries(icons)) {
  const css = `/* Elvish Icon: ${name} */
:root {
  --icon-${name}: ${dataUri};
}
i-thann[icon="${name}"] { -webkit-mask-image: var(--icon-${name}); mask-image: var(--icon-${name}); }
`;
  
  const filePath = path.join(ICONS_DIR, `${name}.css`);
  fs.writeFileSync(filePath, css);
  console.log(`  ✅ icons/${name}.css`);
}

// Generate index that imports all (for convenience)
const allImports = Object.keys(icons).map(name => `@import './${name}.css';`).join('\n');
const indexCss = `/**
 * Elvish Icons - All Icons Bundle
 * 
 * Import this for ALL icons (not tree-shakeable).
 * For selective imports, use individual files:
 *   @import 'elvish/global/icons/home.css';
 *   @import 'elvish/global/icons/star.css';
 */

${allImports}
`;

fs.writeFileSync(path.join(ICONS_DIR, 'index.css'), indexCss);
console.log(`  ✅ icons/index.css (all ${Object.keys(icons).length} icons)`);

// Generate base styles (no icons, just thann[icon] styling)
const baseCss = `/**
 * Elvish Icons - Base Styles Only
 * 
 * Import this ONCE, then add individual icon imports.
 * This provides the base i-thann[icon] styling without any icon data.
 */
...
/* Animations */
i-thann[icon][spin] { animation: thann-icon-spin 1s linear infinite; }
i-thann[icon][spin="slow"] { animation: thann-icon-spin 2s linear infinite; }
i-thann[icon][spin="fast"] { animation: thann-icon-spin 0.5s linear infinite; }
i-thann[icon][pulse] { animation: thann-icon-pulse 1.5s ease-in-out infinite; }

@keyframes thann-icon-spin {
  from { transform: rotate(0deg); }
  to { transform: rotate(360deg); }
}

@keyframes thann-icon-pulse {
  0%, 100% { opacity: 1; }
  50% { opacity: 0.5; }
}
`;

fs.writeFileSync(path.join(ICONS_DIR, 'base.css'), baseCss);
console.log(`  ✅ icons/base.css (base styles only)`);

console.log(`\n✨ Generated ${Object.keys(icons).length} individual icon files!`);
console.log(`
Usage (tree-shakeable):
  /* Import base styles once */
  @import 'elvish/global/icons/base.css';
  
  /* Import only the icons you need */
  @import 'elvish/global/icons/home.css';
  @import 'elvish/global/icons/star.css';
  @import 'elvish/global/icons/check.css';

Usage (all icons):
  @import 'elvish/global/icons/index.css';
`);
